...
function renderAll(){
  ...
  document.getElementById('vp-anga-tabs').innerHTML=Object.keys(ANGA_DEFS).map(key=>{
    ...
    const cur=def.periods.find(p=>+jdToDate(p.startJD)<=+now&&+now<+jdToDate(p.endJD))||def.periods[0];
  ...
  (function renderAngaPanel(){
    ...
    const curIdx=periods.findIndex(p=>+jdToDate(p.startJD)<=+now&&+now<+jdToDate(p.endJD));
    ...
    periods.forEach((p,i)=>{
      ...
      const relPill=relHTML(psd,ped,now);
