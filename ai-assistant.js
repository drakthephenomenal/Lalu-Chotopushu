...
try {
  ...
  if (!res.ok) { const errJson = await res.json().catch(()=>({})); throw new Error('HTTP ' + res.status + ' — ' + (errJson.error || errJson.detail || 'unknown')); }
  ...
  this.showTyping(false);
  this.addMsg('ai', '🙏 Error: ' + err.message);
}
...
css.textContent = `#rjai-toggle{display:flex;align-items:center;gap:9px;width:100%;margin:0 0 14px;padding:11px 16px;background:rgba(109,184,255,0.06);border:1px solid rgba(109,184,255,0.22);border-radius:14px;cursor:pointer;color:var(--a2);font-family:'Inter',sans-serif;font-size:13px;font-weight:500;-webkit-tap-highlight-color:transparent;transition:background 0.15s;} #rjai-toggle:active{background:rgba(109,184,255,0.13);} .rjai-tgl-icon{width:30px;height:30px;border-radius:8px;flex-shrink:0;background:linear-gradient(135deg,#3a5a8a,#6DB8FF);display:flex;align-items:center;justify-content:center;font-size:12px;color:#fff;font-weight:700;letter-spacing:-0.3px;} .rjai-tgl-txt{flex:1;text-align:left;} .rjai-tgl-lbl{font-size:13px;font-weight:600;color:var(--a2);} .rjai-tgl-sub{font-size:10px;color:rgba(255,255,255,0.35);margin-top:1px;letter-spacing:0.5px;} .rjai-tgl-arr{font-size:11px;color:rgba(255,255,255,0.3);flex-shrink:0;} #rjai-panel{display:none;position:fixed;inset:0;z-index:9900;flex-direction:column;background:#060D1F;transform:translateY(100%);transition:transform 0.32s cubic-bezier(0.32,0.72,0,1);font-family:'Inter',sans-serif;padding-bottom:env(safe-area-inset-bottom,0px);} #rjai-panel.rjai-open{transform:translateY(0);} .rjai-hdr{display:flex;align-items:center;gap:10px;padding:14px 16px 11px;border-bottom:1px solid rgba(109,184,255,0.15);background:rgba(0,0,0,0.5);flex-shrink:0;padding-top:max(14px,calc(env(safe-area-inset-top,0px) + 14px));} .rjai-hdr-ic{width:34px;height:34px;border-radius:9px;flex-shrink:0;background:linear-gradient(135deg,#3a5a8a,#6DB8FF);display:flex;align-items:center;justify-content:center;font-size:12px;color:#fff;font-weight:700;} .rjai-hdr-info{flex:1;} .rjai-hdr-name{font-size:14px;font-weight:700;color:var(--a2);} .rjai-hdr-desc{font-size:10px;color:rgba(255,255,255,0.35);letter-spacing:0.8px;text-transform:uppercase;margin-top:1px;} .rjai-hdr-cls{background:rgba(255,255,255,0.07);border:1px solid rgba(255,255,255,0.12);color:rgba(255,255,255,0.55);border-radius:9px;padding:7px 13px;font-size:13px;cursor:pointer;flex-shrink:0;font-family:'Inter',sans-serif;} .rjai-hdr-cls:active{background:rgba(255,255,255,0.14);} .rjai-quick{display:flex;gap:7px;padding:9px 14px 8px;overflow-x:auto;flex-shrink:0;border-bottom:1px solid rgba(109,184,255,0.08);scrollbar-width:none;-webkit-overflow-scrolling:touch;} .rjai-quick::-webkit-scrollbar{display:none;} .rjai-qb{flex-shrink:0;white-space:nowrap;padding:6px 13px;border-radius:20px;background:rgba(109,184,255,0.07);border:1px solid rgba(109,184,255,0.2);color:rgba(109,184,255,0.85);font-size:11px;cursor:pointer;font-family:'Inter',sans-serif;-webkit-tap-highlight-color:transparent;} .rjai-qb:active{background:rgba(109,184,255,0.18);} #rjai-feed{flex:1;overflow-y:auto;padding:14px 14px 8px;display:flex;flex-direction:column;gap:13px;-webkit-overflow-scrolling:touch;scrollbar-width:thin;scrollbar-color:rgba(109,184,255,0.15) transparent;} .rjai-msg{display:flex;gap:8px;align-items:flex-end;} .rjai-ai{flex-direction:row;} .rjai-user{flex-direction:row-reverse;} .rjai-av{width:27px;height:27px;border-radius:8px;flex-shrink:0;background:linear-gradient(135deg,#3a5a8a,#6DB8FF);display:flex;align-items:center;justify-content:center;font-size:11px;color:#fff;font-weight:700;} .rjai-bub{max-width:84%;padding:10px 13px;border-radius:16px;font-size:13.5px;line-height:1.58;word-break:break-word;} .rjai-ai .rjai-bub{background:rgba(255,255,255,0.055);border:1px solid rgba(109,184,255,0.18);color:rgba(255,255,255,0.92);border-bottom-left-radius:4px;} .rjai-user .rjai-bub{background:rgba(109,184,255,0.14);border:1px solid rgba(109,184,255,0.3);color:#dff0ff;border-bottom-right-radius:4px;} .rjai-bub strong{color:#6DB8FF;font-weight:600;} .rjai-bub code{background:rgba(0,0,0,0.4);padding:1px 5px;border-radius:4px;font-size:11.5px;font-family:monospace;color:#7df0c2;} #rjai-typing{display:none;align-items:flex-end;gap:8px;padding:2px 14px 4px;flex-shrink:0;} .rjai-tbub{background:rgba(255,255,255,0.055);border:1px solid rgba(109,184,255,0.18);border-radius:16px;border-bottom-left-radius:4px;padding:10px 15px;display:flex;gap:5px;align-items:center;} .rjai-dot{width:6px;height:6px;border-radius:50%;background:rgba(109,184,255,0.55);animation:rjai-b 1.3s infinite ease-in-out;} .rjai-dot:nth-child(2){animation-delay:0.18s;} .rjai-dot:nth-child(3){animation-delay:0.36s;} @keyframes rjai-b{0%,60%,100%{transform:translateY(0);opacity:0.45;}30%{transform:translateY(-5px);opacity:1;}} .rjai-inp-row{display:flex;gap:8px;padding:9px 14px 12px;border-top:1px solid rgba(109,184,255,0.1);background:rgba(0,0,0,0.35);flex-shrink:0;align-items:flex-end;} #rjai-inp{flex:1;background:rgba(255,255,255,0.06);border:1px solid rgba(109,184,255,0.22);border-radius:20px;padding:10px 14px;color:#fff;font-size:14px;font-family:'Inter',sans-serif;outline:none;resize:none;min-height:40px;max-height:96px;line-height:1.45;-webkit-appearance:none;} #rjai-inp::placeholder{color:rgba(255,255,255,0.28);} #rjai-inp:focus{border-color:rgba(109,184,255,0.45);} #rjai-send{width:42px;height:42px;border-radius:50%;flex-shrink:0;background:linear-gradient(135deg,#3a5a8a,#6DB8FF);border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;font-size:16px;color:#fff;transition:transform 0.12s,opacity 0.2s;-webkit-tap-highlight-color:transparent;} #rjai-send:active{transform:scale(0.88);} #rjai-send:disabled{opacity:0.35;cursor:default;}`;
