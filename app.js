...
function showGPSPrompt() {
  // Only show if never asked before and geolocation available
  if (!navigator.geolocation) return;
  if (localStorage.getItem('rjap_gps_asked')) return;
  const el = document.getElementById('gpsPrompt');
  if (!el) return;
  el.style.display = 'flex';
  // Animate up
  const sheet = el.querySelector('div');
  if (sheet) { sheet.style.transform = 'translateY(100%)'; sheet.style.transition = 'transform 0.4s cubic-bezier(.4,.0,.2,1)'; }
  requestAnimationFrame(() => requestAnimationFrame(() => {
    if (sheet) sheet.style.transform = 'translateY(0)';
  }));
}

function requestGPS(allow) {
  localStorage.setItem('rjap_gps_asked', '1');
  const el = document.getElementById('gpsPrompt');
  if (el) el.style.display = 'none';
  if (allow && navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(
      pos => {
        // Store for use by Brahma Muhurta / panchang calcs
        localStorage.setItem('rjap_lat', pos.coords.latitude);
        localStorage.setItem('rjap_lon', pos.coords.longitude);
        // Trigger Brahma Muhurta recalc if function exists
        if (typeof updateBrahmaMuhurta === 'function') updateBrahmaMuhurta();
        if (typeof initPanchang === 'function') initPanchang();
        toast('📍 Location saved for auspicious timings');
      },
      () => { toast('Location unavailable — using default timings'); },
...
window.addEventListener('beforeinstallprompt', e => {
  ...
  if (!localStorage.getItem('rjap_pwa_dismissed')) {
...
window.addEventListener('appinstalled', () => {
  ...
  localStorage.setItem('rjap_pwa_dismissed', '1');
...
function dismissPWA() {
  localStorage.setItem('rjap_pwa_dismissed', '1');
  const el = document.getElementById('pwaPrompt');
  if (el) { el.style.opacity = '0'; el.style.transform = 'translateY(20px)'; setTimeout(() => { el.style.display = 'none'; }, 350); }
}

// ── Trigger GPS prompt on first load (after 1.5s) ──
window.addEventListener('load', () => {
  setTimeout(showGPSPrompt, 1500);
});
