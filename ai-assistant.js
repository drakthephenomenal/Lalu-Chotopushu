...
//  Powered by Gemini · Voice + Chat · Data manipulation + Undo
...
var JA = {
  addJap: function(count, date, mode) {
    ...
    App.save(); if(typeof App.ua==='function')App.ua(); fbDebouncedPush();
    if(typeof logActivity==='function') logActivity({ t:'jarvis_edit', ts:Date.now(), action:'addJap', date:tk, mode:m, added:count });
    pushUndo('Added '+count+' '+m+' jap to '+tk, function(){ hist[tk]=prev; App.save(); if(typeof App.ua==='function')App.ua(); fbDebouncedPush(); });
  ...
  setJap: function(count, date, mode) {
    ...
    App.save(); if(typeof App.ua==='function')App.ua(); fbDebouncedPush();
    pushUndo('Set '+m+' jap '+tk+' to '+count, function(){ hist[tk]=prev; App.save(); if(typeof App.ua==='function')App.ua(); fbDebouncedPush(); });
  ...
  addStotram: function(id, count) {
    ...
    pushUndo('Add stotram '+id, function(){ App.S.stotrams[id][tk]=prev; App.save(); fbDebouncedPush(); });
  ...
  setDailyTarget: function(n, mode) {
    ...
    App.save(); if(typeof App.ua==='function')App.ua(); fbDebouncedPush();
    pushUndo('Daily target', function(){ if(mode==='rv') App.S.dtRV=prev; else App.S.dt=prev; App.save(); if(typeof App.ua==='function')App.ua(); fbDebouncedPush(); });
  ...
  setLifetimeTarget: function(n) {
    ...
    App.S.lt = n;
    App.save(); if(typeof App.ua==='function')App.ua(); fbDebouncedPush();
    pushUndo('Lifetime target', function(){ App.S.lt=prev; App.save(); if(typeof App.ua==='function')App.ua(); fbDebouncedPush(); });
...
var Jarvis = {
  ...
  send: function(msg) {
    ...
    var sp = 'You are Jarvis, the AI assistant for Radha Naam Jap — a spiritual practice app for devotees of Radha Rani.\n\n'
      ...
      +'YOU CAN MODIFY APP DATA. When user asks to change data, include an action block:\n'
      +'```action\n{"fn":"addJap","args":[108,"2025-05-03","radha"]}\n```\n'
      +'FUNCTIONS AVAILABLE:\n'
      +'- addJap(count, date_optional, mode_optional) — add jap counts\n'
      +'- setJap(count, date, mode) — set exact jap for a date\n'
      +'- markBrahma(date, "m"|"b") — mark brahmacharya maintained/broken\n'
      +'- addStotram(stotramId, count) — log stotram recitations\n'
      +'- setDailyTarget(n, mode_optional) — set daily target\n'
      +'- setLifetimeTarget(n) — set lifetime jap target\n'
      +'- undo() — undo last change\n'
      +'- undoHistory() — show what can be undone\n\n'
  ...
  mount:function(){
    ...
    if(vs){
      ...
      btn.innerHTML='<div class="rjai-tgl-ic">'+icon+'</div><div class="rjai-tgl-txt"><div class="rjai-tgl-lbl">Jarvis — AI Assistant</div><div class="rjai-tgl-sub">Gemini · Voice + Chat · Edit data</div></div><span class="rjai-tgl-arr">▶</span>';
    ...
    panel.innerHTML='<div class="rjai-hdr"><div class="rjai-hdr-ic">'+icon+'</div><div class="rjai-hdr-info"><div class="rjai-hdr-name">Jarvis</div><div class="rjai-hdr-desc">Sadhana AI · Gemini</div></div><button class="rjai-cls" onclick="Jarvis.close()">✕ Close</button></div>'
