...
function renderSt() {
  ...
  // Show developer notification button in Settings for devs
  const devNotifWrap = document.getElementById("devNotifBtnWrap");
  if (devNotifWrap) devNotifWrap.style.display = isDeveloper() ? "block" : "none";
...
// ═══════════════════════════════════════════════════════
// COMMUNITY CHAT — Peer-to-peer via Firestore
...
window.toggleChat = function() {
  ...
  if (chevron) chevron.style.transform = opening ? 'rotate(90deg)' : 'rotate(0deg)';
  if (opening) {
    const signInNote = document.getElementById('chatSignInNote');
    ...
    if (window._chatUnsubscribe) { try { window._chatUnsubscribe(); } catch(_){} window._chatUnsubscribe = null; }
  }
};

function loadChatMessages() {
  if (!fbDb) return;
  if (window._chatUnsubscribe) { try { window._chatUnsubscribe(); } catch(_){} }
  const msgDiv = document.getElementById('chatMessages');
  if (msgDiv) msgDiv.innerHTML = '<div style="text-align:center;color:var(--td);font-size:12px;padding:12px">Loading…</div>';
  window._chatUnsubscribe = fbDb.collection('community_chat')
    .orderBy('ts', 'desc')
    .limit(60)
    .onSnapshot(function(snap) {
      const msgs = [];
      snap.forEach(function(doc) { msgs.push({ id: doc.id, ...doc.data() }); });
      msgs.reverse();
      renderChatMessages(msgs);
    }, function() {
      if (msgDiv) msgDiv.innerHTML = '<div style="text-align:center;color:var(--rl);font-size:12px;padding:12px">Could not load chat</div>';
    });
}
...
function renderChatMessages(msgs) {
  ...
  if (!msgDiv) return;
  if (!msgs.length) {
    msgDiv.innerHTML = '<div style="text-align:center;color:var(--td);font-size:12px;padding:16px">No messages yet. Start the conversation! 🙏</div>';
    return;
  }
  const myUid = fbUser && fbUser.uid;
  msgDiv.innerHTML = msgs.map(function(m) {
    const isMe = m.uid === myUid;
    const isDev = DEV_IDS.map(function(e){return e.toLowerCase();}).includes((m.email||'').toLowerCase());
    const nameColor = isDev ? 'var(--gold)' : isMe ? '#7dd3fc' : 'var(--tl)';
    const devBadge = isDev ? '<span style="font-size:9px;background:rgba(255,215,0,0.2);color:var(--gold);border-radius:4px;padding:1px 5px;margin-left:4px">DEV</span>' : '';
    const align = isMe ? 'flex-end' : 'flex-start';
    const bubbleBg = isMe ? 'rgba(109,184,255,0.13)' : 'rgba(255,255,255,0.06)';
    const bubbleBorder = isMe ? '1px solid rgba(109,184,255,0.25)' : '1px solid rgba(255,255,255,0.08)';
    let timeStr = '';
    try {
      const d = m.ts && m.ts.toDate ? m.ts.toDate() : new Date(m.ts);
      const h = d.getHours(), min = d.getMinutes();
      timeStr = (h%12||12)+':'+String(min).padStart(2,'0')+' '+(h<12?'AM':'PM');
    } catch(_) {}
    return '<div style="display:flex;flex-direction:column;align-items:'+align+';max-width:88%">' +
      '<div style="font-size:10px;color:'+nameColor+';margin-bottom:2px;padding:0 4px">'+escHtml(m.name||'Devotee')+devBadge+'</div>' +
      '<div style="background:'+bubbleBg+';border:'+bubbleBorder+';border-radius:12px;padding:8px 12px;color:var(--tl);font-size:13px;line-height:1.5;word-break:break-word">'+escHtml(m.text||'')+'</div>' +
      '<div style="font-size:9px;color:var(--td);margin-top:2px;padding:0 4px">'+timeStr+'</div>' +
      '</div>';
  }).join('');
...
window.sendChatMessage = async function() {
  if (!fbUser || !fbDb) { toast('Please sign in to chat'); return; }
  const inp = document.getElementById('chatInput');
  if (!inp) return;
  const text = inp.value.trim();
  if (!text) return;
  if (text.length > 500) { toast('Message too long (max 500 chars)'); return; }
  inp.value = '';
  inp.disabled = true;
  try {
    let name = (App.S.lbDisplayName || '').trim();
    if (!name) name = (fbUser.displayName || (fbUser.email||'').split('@')[0] || 'Devotee').slice(0,30);
    await fbDb.collection('community_chat').add({
      uid: fbUser.uid,
      email: fbUser.email || '',
      name: name,
      text: text,
...
// Collection: dev_notifications (active:true docs)
// ═══════════════════════════════════════════════════════
...
(function() {
  try {
    const saved = JSON.parse(localStorage.getItem('rjap_seen_notifs') || '[]');
    _notifSeenIds = new Set(saved);
  } catch(_) {}
})();

function _saveSeenNotifs() {
  try {
    localStorage.setItem('rjap_seen_notifs', JSON.stringify(Array.from(_notifSeenIds).slice(-100)));
  } catch(_) {}
}

function initDevNotifications() {
  if (!fbDb) return;
  if (window._notifUnsubscribe) { try { window._notifUnsubscribe(); } catch(_){} }
  window._notifUnsubscribe = fbDb.collection('dev_notifications')
    .where('active', '==', true)
    .orderBy('ts', 'desc')
    .limit(10)
    .onSnapshot(function(snap) {
      snap.forEach(function(doc) {
        const id = doc.id;
        if (_notifSeenIds.has(id)) return;
        const data = doc.data();
        if (!data.text) return;
        _notifSeenIds.add(id);
        _saveSeenNotifs();
        showDevNotification(id, data);
      });
    }, function(err) { console.warn('Dev notif listener:', err.message); });
}

function showDevNotification(id, data) {
  const existing = document.getElementById('devNotifPopup');
  if (existing) existing.remove();
  const popup = document.createElement('div');
  popup.id = 'devNotifPopup';
  popup.style.cssText = 'position:fixed;top:0;left:0;right:0;z-index:99999;display:flex;justify-content:center;padding:12px 16px 0;pointer-events:none;animation:devNotifSlideIn 0.4s cubic-bezier(0.34,1.56,0.64,1) forwards';
  popup.innerHTML =
    '<div style="pointer-events:auto;max-width:480px;width:100%;background:linear-gradient(135deg,rgba(10,21,53,0.97),rgba(6,13,31,0.98));border:1px solid rgba(255,215,0,0.35);border-radius:16px;padding:14px 16px;box-shadow:0 8px 32px rgba(0,0,0,0.6)">' +
      '<div style="display:flex;align-items:flex-start;gap:10px">' +
        '<div style="font-size:22px;flex-shrink:0;margin-top:1px">'+(data.icon||'📢')+'</div>' +
        '<div style="flex:1;min-width:0">' +
          '<div style="color:var(--gold);font-size:11px;font-weight:600;letter-spacing:1px;text-transform:uppercase;margin-bottom:3px">Developer Message'+(data.title?' — '+escHtml(data.title):'')+'</div>' +
          '<div style="color:var(--tl);font-size:13px;line-height:1.5;word-break:break-word">'+escHtml(data.text)+'</div>' +
          (data.link ? '<a href="'+escHtml(data.link)+'" target="_blank" style="display:inline-block;margin-top:6px;color:#7dd3fc;font-size:11px;text-decoration:underline">'+escHtml(data.linkLabel||data.link)+'</a>' : '') +
        '</div>' +
        '<button onclick="dismissDevNotif(\''+id+'\')" style="flex-shrink:0;background:none;border:none;color:var(--td);font-size:20px;cursor:pointer;padding:0 0 0 6px;line-height:1">×</button>' +
      '</div>' +
    '</div>';
  document.body.appendChild(popup);
  setTimeout(function() { dismissDevNotif(id); }, 12000);
}

window.dismissDevNotif = function(id) {
  const popup = document.getElementById('devNotifPopup');
  if (popup) {
    popup.style.transition = 'opacity 0.3s,transform 0.3s';
    popup.style.opacity = '0';
    popup.style.transform = 'translateY(-16px)';
    setTimeout(function() { if (popup.parentNode) popup.remove(); }, 320);
  }
};

window.openDevNotifComposer = function() {
  if (!isDeveloper()) { toast('Access denied'); return; }
  const existing = document.getElementById('devNotifComposer');
  if (existing) { existing.remove(); return; }
  const panel = document.createElement('div');
  panel.id = 'devNotifComposer';
  panel.style.cssText = 'position:fixed;bottom:80px;left:50%;transform:translateX(-50%);z-index:9999;width:min(420px,94vw);background:rgba(10,21,53,0.98);border:1px solid rgba(255,215,0,0.3);border-radius:16px;padding:16px;box-shadow:0 8px 32px rgba(0,0,0,0.7)';
  panel.innerHTML =
    '<div style="color:var(--gold);font-size:13px;font-weight:700;margin-bottom:12px">📢 Send Developer Notification</div>' +
    '<input id="devNotifTitle" placeholder="Title (optional)" style="width:100%;box-sizing:border-box;background:rgba(255,255,255,0.07);border:1px solid rgba(255,215,0,0.2);border-radius:9px;padding:9px 12px;color:var(--tl);font-size:13px;margin-bottom:8px;outline:none">' +
    '<textarea id="devNotifText" rows="3" placeholder="Message text…" style="width:100%;box-sizing:border-box;background:rgba(255,255,255,0.07);border:1px solid rgba(255,215,0,0.2);border-radius:9px;padding:9px 12px;color:var(--tl);font-size:13px;resize:none;outline:none;margin-bottom:8px"></textarea>' +
    '<div style="display:flex;gap:8px;margin-bottom:8px">' +
      '<input id="devNotifIcon" placeholder="Icon 📢" style="flex:0 0 80px;background:rgba(255,255,255,0.07);border:1px solid rgba(255,215,0,0.2);border-radius:9px;padding:9px 10px;color:var(--tl);font-size:13px;outline:none">' +
      '<input id="devNotifLink" placeholder="Link URL (optional)" style="flex:1;background:rgba(255,255,255,0.07);border:1px solid rgba(255,215,0,0.2);border-radius:9px;padding:9px 12px;color:var(--tl);font-size:13px;outline:none">' +
    '</div>' +
    '<div style="display:flex;gap:8px">' +
      '<button onclick="sendDevNotif()" style="flex:1;padding:10px;border-radius:10px;border:none;background:linear-gradient(135deg,rgba(255,215,0,0.3),rgba(255,180,0,0.2));color:var(--gold);font-size:13px;font-weight:600;cursor:pointer">Send to All Users</button>' +
      '<button onclick="document.getElementById(\'devNotifComposer\').remove()" style="padding:10px 16px;border-radius:10px;border:1px solid rgba(255,255,255,0.1);background:transparent;color:var(--td);font-size:13px;cursor:pointer">Cancel</button>' +
    '</div>' +
    '<div id="devNotifFeedback" style="font-size:11px;color:var(--green);margin-top:8px;text-align:center"></div>';
  document.body.appendChild(panel);
};

window.sendDevNotif = async function() {
  if (!isDeveloper() || !fbDb) { toast('Access denied'); return; }
  const text = (document.getElementById('devNotifText')||{}).value?.trim();
  if (!text) { toast('Message text required'); return; }
  const title = (document.getElementById('devNotifTitle')||{}).value?.trim() || '';
  const icon = (document.getElementById('devNotifIcon')||{}).value?.trim() || '📢';
  const link = (document.getElementById('devNotifLink')||{}).value?.trim() || '';
  const fb2 = document.getElementById('devNotifFeedback');
  if (fb2) fb2.textContent = 'Sending…';
  try {
    await fbDb.collection('dev_notifications').add({
      title, text, icon,
      link: link || null,
      active: true,
      ts: firebase.firestore.FieldValue.serverTimestamp(),
    ...
    if (fb2) fb2.textContent = '✓ Notification sent!';
    setTimeout(function() {
      const c = document.getElementById('devNotifComposer');
      if (c) c.remove();
    }, 1800);
  } catch(e) {
    if (fb2) fb2.textContent = 'Error: ' + e.message;
  }
};

// Start notification listener after Firebase auth is ready
// Patch into existing auth callback or poll for fbUser
(function() {
  let _notifStarted = false;
  function _tryStartNotifs() {
    if (_notifStarted) return;
    if (typeof fbUser !== 'undefined' && fbUser && typeof fbDb !== 'undefined' && fbDb) {
      _notifStarted = true;
      initDevNotifications();
    }
  }
  // Try at intervals — auth can complete at any point
  var _notifPoll = setInterval(function() {
    _tryStartNotifs();
    if (_notifStarted) clearInterval(_notifPoll);
  }, 1500);
  setTimeout(function() { clearInterval(_notifPoll); }, 30000); // give up after 30s
})();
